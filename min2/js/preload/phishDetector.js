...
function checkPhishingStatus() {
  ...
  function isSensitive(form) {
    // checks if a form is asking for sensitive information
    ...
    if (
      form.querySelectorAll('input[type=text], input[type=password]').length ===
      2
    ) {
      debugPhishing(
        'possibly sensitive form, checking but increasing minScore'
      );
    ...
    // myglobal.empty forms can be misleading

    if (!form.querySelector('input')) {
      debugPhishing(
        'myglobal.empty form found, checking but not counting as sensitive'
      );
  ...
  var sensitiveFormWords = [
    ...
    'date of birth',
    'joint return',
  ]; // word commonly found in forms that ask for personal information
  var whitelistedDomains = [
    'adobeid-na1.services.adobe.com',
    'www.zdnet.com',
    'www.discover.com',
  ]; // a whitelist of things we mistakenly think are bad. These should be fixed eventually, but for now a whitelist will work.
  ...
  if (loc.split('#')[0].split('/').length > 6) {
    ...
    phishingScore += Math.max(
      Math.min(loc.split('/').length * 0.05, 0.75),
      0.25
    );
  }
  ...
  if (
    window.location.hostname.replace('www.', '').split('.').length > 3 &&
    window.location.hostname.length > 20
  ) {
  ...
  if (window.location.pathname.length > 25) {
    ...
    phishingScore += Math.min(
      0.05 + 0.002 * window.location.pathname.length,
      0.25
    );
  }

  if (
    isTrustedDomainEnding &&
    window.location.pathname.length < 20 &&
    window.location.hostname.replace('www.', '').length < 18
  ) {
    debugPhishing('short root domain found, increasing minScore');
    minPhishingScore +=
      0.3 +
      0.05 * (18 - window.location.hostname.length) -
      0.01 * window.location.pathname.length;
  }
  ...
  if (forms) {
    for (var i = 0; i < forms.length; i++) {
      ...
      if (!fa || loc.split('?')[0].indexOf(fa.split('?')[0]) !== -1) {
        // we also check if the form just submits to the same page with a different query string
      ...
      if (
        ...
        form.getAttribute('onkeydown') ||
        form.getAttribute('onkeypress') ||
        form.getAttribute('onkeyup')
      ) {
      ...
      if (
        fa.indexOf('javascript:') !== 0 &&
        isThirdParty(
          getRootDomain(window.location.hostname),
          getRootDomain(aTest.hostname)
        )
      ) {
  ...
  for (var i = 0; i < links.length; i++) {
    ...
    if (!href || (href === '#' && !links[i].onclick)) {
      // ignore links that have a javascript handler, since these are expected to have an myglobal.empty href
      emptyLinksFound++;
      continue;
    }
  ...
  for (var key in linkDomains) {
    ...
    if (linkDomains[key] > 4 && key && linkDomains[key] / totalLinks > 0.25) {
      debugPhishing(
        'found ' + linkDomains[key] + ' links that point to domain ' + key
      );
  ...
  if (
    (totalLinks > 2 && sameDomainLinks === 0) ||
    (totalLinks > 5 && sameDomainLinks / totalLinks < 0.15)
  ) {
    debugPhishing('links go to external domain');
    phishingScore += Math.min(0.1 + (totalLinks - sameDomainLinks) * 0.1, 0.5);
  }

  // if there are a bunch of myglobal.empty links, increase score
  if (
    emptyLinksFound > 9 ||
    (totalLinks > 2 && emptyLinksFound / totalLinks > 0.5)
  ) {
    debugPhishing('counted ' + emptyLinksFound + ' myglobal.empty links');
  ...
  if (
    totalFormLength > 50 &&
    totalFormLength < 1000 &&
    totalFormLength / totalDocLength < 0.075
  ) {
    ...
    totalFormLength > 50 &&
    totalFormLength < 3500 &&
    totalFormLength / totalDocLength < 0.14
  ) {
  ...
  for (var source in scriptSources) {
    if (
      scriptSources[source] > 2 &&
      scriptSources[source] / totalScripts > 0.75 &&
      scriptSources[source] < 0.95
    ) {
      phishingScore += 0.1;
  ...
  if (phishingScore > minPhishingScore) {
    electron.ipcRenderer.send('phishingDetected', phishingDebugMessages);
  }

  var scanEnd = performance.now();

  debugPhishing(
    'phishing scan took ' + (scanEnd - scanStart) + ' milliseconds'
  );
