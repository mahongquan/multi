...
const downloadManager = {
  ...
  show: function() {
    if (!downloadManager.isShown) {
      ...
      myglobal.webviews.adjustMargin([0, 0, downloadManager.height, 0]);
    }
  },
  hide: function() {
    if (downloadManager.isShown) {
      ...
      myglobal.webviews.adjustMargin([0, 0, downloadManager.height * -1, 0]);
  ...
  onItemDragged: function(path) {
    electron.remote.app.getFileIcon(path, {}, function(err, icon) {
      if (err) {
        console.warn(err);
        return;
      }
      electron.remote.getCurrentWebContents().startDrag({
        file: path,
        icon: icon,
      });
    });
    ...
    dropdown.addEventListener('click', function() {
      ...
      menu.append(
        new electron.remote.MenuItem({
          ...
          click: function() {
            electron.ipcRenderer.send('cancelDownload', downloadItem.path);
  ...
  updateItem: function(downloadItem) {
    ...
    if (downloadItem.status === 'completed') {
      ...
      elements.progress.style.transform =
        'scaleX(' + downloadItem.size.received / downloadItem.size.total + ')';
    }
  },
  initialize: function() {
    ...
    electron.ipcRenderer.on('download-info', function(e, info) {
      if (!info.path) {
