...
var findinpage = {
  ...
  start: function(options) {
    myglobal.webviews.releaseFocus();

    findinpage.input.placeholder = l('searchInPage');

    findinpage.activeWebview = myglobal.webviews.get(tabs.getSelected());
...
findinpage.input.addEventListener('blur', function(e) {
  if (
    !e.relatedTarget ||
    !e.relatedTarget.classList.contains('findinpage-control')
  ) {
    findinpage.end();
  }
});
...
myglobal.webviews.bindEvent('found-in-page', function(e, data) {
...
module.exports = { findinpage: findinpage };
