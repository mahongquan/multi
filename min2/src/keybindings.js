...
electron.ipcRenderer.on('addTask', function() {
  /* new window.tasks can't be created in focus mode */
  if (focusMode.enabled()) {
    focusMode.warn();
    return;
  }
...
settings.get('keyMap', function(keyMapSettings) {
  ...
  defineShortcut('restoreTab', function(e) {
    ...
    browserUI.addTab(
      tabs.add(restoredTab, tabs.getIndex(tabs.getSelected()) + 1),
      {
        enterEditMode: false,
      }
    );
  ...
  defineShortcut('switchToNextTask', function(d) {
    ...
    const currentTaskIdx = window.tasks.indexOf(myglobal.currentTask);

    const nextTask =
      window.tasks.byIndex(currentTaskIdx + 1) || window.tasks.byIndex(0);
  ...
  defineShortcut('switchToPreviousTask', function(d) {
    ...
    const currentTaskIdx = window.tasks.indexOf(myglobal.currentTask),
      taskCount = window.tasks.getLength();

    const previousTask =
      window.tasks.byIndex(currentTaskIdx - 1) ||
      window.tasks.byIndex(window.tasks.getLength() - 1);
