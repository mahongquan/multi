...
var { registerCustomBang } = require('./bangsPlugin');
...
registerCustomBang({
  ...
  fn: function(text) {
    setTimeout(function() {
      ...
      myglobal.webviews.get(tabs.getSelected()).capturePage(function(image) {
        electron.remote.getCurrentWebContents().downloadURL(image.toDataURL());
      });
    }, 400);
  },
});
...
registerCustomBang({
  ...
  fn: function(text) {
    ...
    // no task was specified, show all of the window.tasks
    if (!text) {
      taskOverlay.show();
      return;
    }
...
registerCustomBang({
  ...
  fn: function(text) {
    ...
    setTimeout(function() {
      browserUI.addTask();
      if (text) {
        myglobal.currentTask.name = text;
      }
    }, 600);
  },
});
...
registerCustomBang({
  ...
  fn: function(text) {
    ...
    if (newTask) {
      ...
      var newTask = window.tasks.get(window.tasks.add());
      newTask.name = text;
        ...
        results
          ...
          .forEach(function(result) {
            ...
            if (thisRelativeDate !== lastRelativeDate) {
              var heading = searchbarUtils.createHeading({
