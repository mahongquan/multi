...
var searchbar = {
  ...
  focusItem: function(options) {
    ...
    var allItems = [].slice.call(
      searchbar.el.querySelectorAll('.searchbar-item:not(.unfocusable)')
    );
    var currentItem = searchbar.el.querySelector(
      '.searchbar-item:focus, .searchbar-item.fakefocus'
    );
    ...
    if (currentItem && logicalNextItem) {
      // an item is focused and there is another item after it, move onto the next one
      logicalNextItem.focus();
    } else if (currentItem) {
      // the last item is focused, focus the searchbar again
