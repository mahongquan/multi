...
addTaskButton.addEventListener('click', function(e) {
  browserUI.switchToTask(window.tasks.add());
  taskOverlay.hide();
});
...
taskOverlay.tabDragula.on('drop', function(el, target, source, sibling) {
  ...
  var newTask = window.tasks.get(target.getAttribute('data-task'));
...
/* rearrange window.tasks when they are dropped */
...
taskOverlay.taskDragula.on('drop', function(el, target, source, sibling) {
  ...
  // remove the task from the window.tasks list
  var droppedTask = window.tasks.splice(
    window.tasks.getIndex(droppedTaskId),
    1
  )[0];
