...
var webviewGestures = {
  ...
  zoomWebviewBy: function(tabId, amt) {
    var w = myglobal.webviews.get(tabId);
    w.getZoomFactor(function(existingZoom) {
      var newZoom = Math.min(
        webviewMaxZoom,
        Math.max(webviewMinZoom, existingZoom + amt)
      );
  ...
  resetWebviewZoom: function(tabId) {
    myglobal.webviews.get(tabId).setZoomFactor(1.0);
  },
};
...
myglobal.webviews.bindIPC('wheel-event', function(webview, tabId, e) {
  e = JSON.parse(e);
  ...
  if (beginningScrollLeft === null || beginningScrollRight === null) {
    myglobal.webviews.get(tabs.getSelected()).executeJavaScript(
      `
    (function () {
      var left = 0
      var right = 0
...
module.exports = { webviewGestures: webviewGestures };
