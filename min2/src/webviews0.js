...
function onPageLoad(e) {
  ...
  myglobal.webviews.callAsync(tab, 'getURL', null, function(url) {
    ...
    if (tab === window.tabs.getSelected() && window.tabs.get(tab).url !== url) {
      setTimeout(function() {
...
myglobal.webviews = {
  ...
  getViewBounds: function() {
    if (windowIsFullscreen) {
      ...
      if (myglobal.platformType === 'windows' && !windowIsMaximised) {
        var navbarHeight = 46;
      } else {
        var navbarHeight = 36;
      }
  ...
  add: function(tabId) {
    ...
    if (tabData.url) {
      myglobal.webviews.callAsync(tabData.id, 'loadURL', tabData.url);
    }
  ...
  requestPlaceholder: function(reason) {
    ...
    if (myglobal.webviews.placeholderRequests.length === 1) {
      ...
      var associatedTab = window.tabs.get(myglobal.webviews.selectedId);
      if (img) {
...
myglobal.webviews.bindEvent(
  ...
  function(e, url, frameName, disposition) {
    ...
    var newTab = window.tabs.add(
      {
        url: url,
        private: window.tabs.get(tab).private, // inherit private status from the current tab
      },
      currentIndex + 1
    );
