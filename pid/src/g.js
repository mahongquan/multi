...
let scrollBackground = function() {
    ...
    translateOffset =
      translateOffset < context.canvas.width
        ? translateOffset + translateDelta
        : 0;
    ...
    //Count
    if (
      translateOffset % RECT_WIDTH === 0 &&
      translateOffset >= RECT_WIDTH &&
      gameStart
    ) {
      countRect++;
...
function updateCurrentColor(translateOffset) {
  switch (translateOffset) {
    ...
    case RECT_WIDTH:
      ...
      currentLedgeColor.push(backColor[0]);
      break;
    case RECT_WIDTH * 2:
      currentLedgeColor.shift();
      currentLedgeColor.push(backColor[1]);
      break;
    case RECT_WIDTH * 3:
      currentLedgeColor.shift();
    ...
    case CANVAS_WIDTH:
      currentLedgeColor.shift();
      currentLedgeColor.push(backColor[3]);
      break;
    default:
      break;
  }
...
function updateMyRealColor() {
  //countRect realMyColor myColor
  let leftOffset = calculLeftOffset();
  if (countRect % 10 === 0 && countRect >= 10) {
    let color = myColor.shift();
    if (color) {
      //if color==undefined, then escape it
...
//Sprite  && behavior
//Initalize
let temp = Behavior.fallOnLedge.ledgeRect[0],
  temp2 = Behavior.fallOnLedge.ledgeRect[1],
  temp3 = Behavior.fallOnLedge.ledgeRect[2],
  temp4 = Behavior.fallOnLedge.ledgeRect[3];

temp.top = temp2.top = temp3.top = temp4.top =
  game.context.canvas.height - RECT_HEIGHT;
temp.width = temp2.width = temp3.width = temp4.width = RECT_WIDTH;
temp.height = temp2.height = temp3.height = temp4.height = RECT_HEIGHT;
resetLeft();

let ballSprite = new Sprite('ball', ballPainter, [
  Behavior.moveGravity,
  Behavior.fallOnLedge,
]);
//INITALIZE
ballSprite.top = BALL_LAUNCH_TOP;
ballSprite.left = BALL_LAUNCH_LEFT;
...
Event.listen('GamePause', () => {
  Behavior.PauseHandler(ballSprite);
});
...
game.paintUnderSprites = function() {
  //Draw things other than sprites
