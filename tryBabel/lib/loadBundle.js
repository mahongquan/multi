...
const LAST_PRESET_ENV_NO_SCOPE_VERSION = "2.0.0-beta.2"; // There is a gap in preset-env versions. From 2.0.0-beta.2 to 7.0.0-beta.33. We need to handle it.

const formatBundleVersion = (packageName, version) => {
  if (!version) return "";

  if (packageName === "babel-preset-env-standalone") {
    const isUnregistered = _semver.default.valid(version) && _semver.default.gt(version, LAST_PRESET_ENV_NO_SCOPE_VERSION) && _semver.default.lt(version, SCOPED_PRESET_ENV_VERSION_FROM);
...
const formatBundleName = (packageName, version) => {
  ...
  const isValidOrGreater = _semver.default.valid(version) && _semver.default.gte(version, minimumScopedVersion);

  return isValidOrGreater || version >= 7 ? packageName.replace(/^babel-/, "@babel/") : packageName;
};

async function loadBundle(state, workerApi) {
  const {
    config
  } = state;
  ...
  function doLoad(url, error) {
    ...
    return workerApi.loadScript(url).then(success => {
      if (success) {
        ...
        return Promise.all([// Incoming version might be unspecific (eg "6")
        // Resolve to a more specific version to show in the UI.
        workerApi.getBundleVersion(config.instanceName), workerApi.getAvailablePresets()]).then(([version, presets]) => {
  ...
  if (build) {
    ...
    try {
      if (!isBuildNumeric) {
        ...
        build = await (0, _CircleCI.loadLatestBuildNumberForBranch)(state.circleciRepo, build === "7.0" ? "master" : build);
      }
      ...
      const url = await (0, _CircleCI.loadBuildArtifacts)(state.circleciRepo, regExp, build, doLoad);
      return doLoad(url);
    } catch (ex) {
  ...
  const base = config.baseUrl; // Override preset-env version to 7.x if babel-core 7.x was loaded.
